namespace LevelManagerLoader
{
    ...
    public class LevelManagerWindowEditor : EditorWindow
    {
        ...
        [MenuItem("Tools/Level Manager/Open Config Container")]
        public static void Open()
        {
            Open(LevelManager.GetContainer());
        }

        private void OnEnable()
        {
            m_container = LevelManager.GetContainer();
        ...
        private static void Open(LevelManagerContainer scriptableObject)
        {
            LevelManagerWindowEditor window = GetWindow<LevelManagerWindowEditor>("Level Manager Window");
            window.m_container = scriptableObject;
        }
            ...
            for (int group = 0; group < m_levelGroupsList.arraySize; group++)
            {
                ...
                if (m_open[group])
                {
                    for (int level = 0; level < m_levelsList.arraySize; level++)
                    {
                        ...
                        EditorGUILayout.HelpBox($"{nameEditor} #{level + 1}, Group: {m_container.LevelGroups[group].GroupType.ToString()}", MessageType.None);
